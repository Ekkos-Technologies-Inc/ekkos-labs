...
### Scorecard Environment Variables (`/scorecard`)

To enable the live public scorecard endpoints, set:

- `NEXT_PUBLIC_SUPABASE_URL`
- `SUPABASE_SERVICE_ROLE` (or `SUPABASE_SERVICE_KEY`)
- `SCORECARD_INGEST_TOKEN`

Ingestion endpoint:

```bash
POST /api/scorecard/ingest
```

Auth header:

```bash
x-scorecard-token: <SCORECARD_INGEST_TOKEN>
```
