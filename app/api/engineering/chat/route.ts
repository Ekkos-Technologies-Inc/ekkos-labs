...
import { callGeminiWithTools, DEFAULT_MODEL } from '@ekkos/gemini';
...
You're running on Gemini 2.5 Flash with live MCP tool access to all 11 memory layers.`;
...
async function callGemini(contents: GeminiContent[], tools: unknown[]) {
  const data = await callGeminiWithTools(contents, {
    model: DEFAULT_MODEL,
    tools,
  ...
  if (!data) {
    throw new Error('Gemini unavailable: set GOOGLE_VERTEX_PROJECT and GOOGLE_VERTEX_AI_KEY');
  }
