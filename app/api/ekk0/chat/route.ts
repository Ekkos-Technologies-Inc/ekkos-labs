...
import { callGemini, DEFAULT_MODEL } from "@ekkos/gemini";
...
export async function POST(request: NextRequest) {
  if (!process.env.GOOGLE_VERTEX_PROJECT || !(process.env.GOOGLE_VERTEX_AI_KEY || process.env.GOOGLE_AI_API_KEY)) {
  ...
  try {
    ...
    if (!text) {
      console.error("[ekk0/chat] Gemini returned empty response");
      return NextResponse.json(
